...
: mc_robot_tools::ConnectableRobotModule(MC_DATA_PATH, name),
  prefix_(name.find("85") != std::string::npos ? "robotiq_85" : "robotiq_140" ; "robotiq_hande")
{
  bool fixed = false;
...
{
  return prefix_ + "_base_link";
}

std::vector<std::string> RobotiqGripperRobotModule::collisionLinks() const
{
  return {prefix_ + "_base_link",
...
sva::PTransformd RobotiqGripperRobotModule::defaultMountingTransform() const
{
